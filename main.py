...
def find_it(listData,key):
	for index, sublist in enumerate(listData):
		if sublist[0] == key:
			return index
	return -1

def indexer(dataAllList):
	term_and_docId_list=[]
	index=[]
	prevIteam=""
	prevId=-1
	...
	for d in term_and_docId_list:
		...
		if iteam==prevIteam and prevId!=docId:
			#pos=find_it(index,term)
			pos=len(index)-1
			index[pos][1]+=1
			(index[pos]).append(docId)
		elif iteam!=prevIteam:
			index.append([iteam,1,docId])
		prevIteam=iteam
		prevId=docId
	return index


data=["This maker is a beautful look eggy","good to see you beautful","Are you lucky are not"]
